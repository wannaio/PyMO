...
from dm.utils.PyMO.pymo.rotation_tools import Rotation, euler2expmap, euler2expmap2, expmap2euler, euler_reorder, unroll
from dm.utils.PyMO.pymo.Quaternions import Quaternions
from dm.utils.PyMO.pymo.Pivots import Pivots

class MocapParameterizer(BaseEstimator, TransformerMixin):
    def __init__(self, param_type = 'euler', pymo_verbose=False):
        '''
    ...
    def transform(self, X, y=None):
        self.pymo_verbose: print("MocapParameterizer: " + self.param_type)
...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    '''
    def __init__(self, joints, include_root=False, pymo_verbose=False):
        self.joints = joints
    ...
    def transform(self, X, y=None):
        self.pymo_verbose: print("JointSelector")
        Q = []
        for track in X:
            t2 = track.clone()
...
class Numpyfier(BaseEstimator, TransformerMixin):
    ...
    '''
    def __init__(self, pymo_verbose=False):
        self.pymo_verbose = pymo_verbose
    ...
    def transform(self, X, y=None):
        self.pymo_verbose: print("Numpyfier")
        Q = []
...
class RootTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        self.pymo_verbose: print("RootTransformer")
        Q = []
...
class DownSampler(BaseEstimator, TransformerMixin):
    def __init__(self, tgt_fps, keep_all=False, pymo_verbose=False):
    ...
    def fit(self, X, y=None):    
        # raise Exception("fit is not implemented for this transformer")
        return self
    
    def transform(self, X, y=None):
        # raise Exception("transform is not implemented for this transformer")
        Q = []
        ...
        for track in X:
            ...
            if orig_fps%self.tgt_fps!=0:
                if self.pymo_verbose: print("error orig_fps (" + str(orig_fps) + ") is not dividable with tgt_fps (" + str(self.tgt_fps) + ")")
                else: pass
            else:
                if self.pymo_verbose: print("downsampling with rate: " + str(rate))
                else: pass
    ...
    def inverse_transform(self, X, copy=None):
    #   raise Exception("inverse_transform is not implemented for this transformer")  
      return X
